...
const favouriteAuthor = async function (authorName: string): Promise<'favourite' | 'un_favourite'> {
  const dirPath = path.resolve(__dirname, `../../db/user/favouriteAuthor`)
...
const favouriteAuthorList = async function (): Promise<string[]> {
  const filePath = path.resolve(__dirname, `../../db/user/favouriteAuthor/data.json`)
  // no file.
  if (!fs.existsSync(filePath)) {
    return []
  }
...
export { favouriteAuthor, favouriteAuthorList }